...
class DDPM(pl.LightningModule):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        self.register_buffer('posterior_mean_coef1', to_torch(
            betas * np.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod))) # coef to x_0
        self.register_buffer('posterior_mean_coef2', to_torch(
            (1. - alphas_cumprod_prev) * np.sqrt(alphas) / (1. - alphas_cumprod))) # coef to x_t
    ...
    def q_sample(self, x_start, t, noise=None):
        ##import ipdb; ipdb.set_trace()
        noise = default(noise, lambda: torch.randn_like(x_start)) # TODO not necessary if noise!=None
        return (extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start +
                extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise)
        # x_t = sqrt(alpha_t_bar) * x_0 + sqrt(1 - alpha_t_bar) * epsilon # from x_0 to x_t
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            x = x.to(self.device)
            encoder_posterior = self.encode_first_stage(x) # use autoencoder's encode function!
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.parameterization == "x0":
            ...
            target = noise # 特别需要注意，这里的Noise，作为target，并且用来扰动x_0从而得到x_t！逻辑没有变！
        else:
            raise NotImplementedError()
